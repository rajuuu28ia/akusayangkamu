...
async def check(self, username):
    ...
    # Gunakan asyncio untuk menghindari sleep blocking
    result = await asyncio.to_thread(self.check_fragment_api, username.lower())
    await asyncio.sleep(1)  # Delay kecil agar tidak kena rate limit
...
async def batch_check(self, usernames, batch_size=5):
    ...
    for i in range(0, len(usernames), batch_size):
        ...
        tasks = [self.check(username) for username in batch]
        batch_results = await asyncio.gather(*tasks)
        results.extend(batch_results)
        await asyncio.sleep(2)  # Delay antar batch untuk menghindari rate limit

    return results

def run(self, multithread=True):
    loop = asyncio.get_event_loop()
    results = loop.run_until_complete(self.batch_check(list(self.usernames), batch_size=10))
    return results
self, username):
        msg = f'@{username} '
        if not re.compile(r'^[a-zA-Z][a-zA-Z0-9_]{4,31}$').match(username):
